...
const dbUtilsFactory = function dbUtilsFactory(conf) {
  ...
  const makeQuery = async function makeQuery({ sql, values }) {
    const con = await newConnection();
    const rows = await executeQuery(con, { sql, values });
    closeConnection(con);
    return rows;
  };
